...
test.describe('guided test of fate', () => {
	test('the GM calls a test, the acting player buys favor, draws and resolves it, and the whole table sees the outcome', async ({
		...
		await expect(gmPage.getByTestId('declare-test')).toBeVisible({ timeout: 4000 });
		...
		for (const page of [gmPage, playerAPage, playerBPage]) {
			await expect(page.getByTestId('test-of-fate-panel')).toBeVisible({ timeout: 4000 });
			await expect(page.getByTestId('test-actor')).toHaveText('Isolde Fen', { timeout: 4000 });
		...
		await expect(playerAPage.getByTestId('draw-test-card')).toBeVisible({ timeout: 4000 });
		...
		await expect(playerAPage.getByTestId('test-modifier')).toContainText('Resolve', { timeout: 4000 });
		...
		for (const page of [gmPage, playerAPage, playerBPage]) {
			await expect(page.getByTestId('test-initial-card')).toBeVisible({ timeout: 4000 });
			await expect(page.getByTestId('test-outcome')).toBeVisible({ timeout: 4000 });
		}
		...
		await expect(gmPage.getByTestId('complete-test')).toBeVisible({ timeout: 4000 });
		...
		await expect(gmPage.getByTestId('declare-test')).toBeVisible({ timeout: 4000 });
		await expect(playerAPage.getByTestId('test-outcome')).toHaveCount(0, { timeout: 4000 });
	...
	test('a group test runs both subtests through the same panel and publishes one hit total', async ({ browser }) => {
		...
		await expect(gmPage.getByTestId('declare-group-test')).toBeVisible({ timeout: 4000 });
		...
		await expect(candidates).toHaveCount(2, { timeout: 4000 });
		...
		for (const page of [gmPage, playerAPage, playerBPage]) {
			await expect(page.getByTestId('group-test-panel')).toBeVisible({ timeout: 4000 });
			await expect(page.getByTestId('group-most')).not.toBeEmpty({ timeout: 4000 });
		...
		for (const [index, who] of [mostName, leastName].entries()) {
			...
			await clickCommand(gmPage, gmPage.getByTestId('settle-favor'));
			await expect(actorPage.getByTestId('draw-test-card')).toBeVisible({ timeout: 6000 });
			...
			await expect(gmPage.getByTestId('advance-group-test')).toBeVisible({ timeout: 6000 });
			await clickCommand(gmPage, gmPage.getByTestId('advance-group-test'));

			// Each finished subtest is listed for the table.
			await expect(gmPage.getByTestId('group-subtests').locator('li')).toHaveCount(index + 1, { timeout: 6000 });
		}

		// --- One hit total and one band, visible to everyone ---
		for (const page of [gmPage, playerAPage, playerBPage]) {
			await expect(page.getByTestId('group-outcome')).toBeVisible({ timeout: 6000 });
