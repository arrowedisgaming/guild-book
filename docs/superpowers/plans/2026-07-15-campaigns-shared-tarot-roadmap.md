...
| D5 | Does the app implement rules the game does not have? | **Resolved 2026-07-15** | No — anything the rulebook does not contain is removed at the specification, before it can reach content JSON or the engine. Applied to "shield Initiative replacement" (§8.6/§8.7), which does not exist: the real shield rules are Notch absorption and an attack tie-break, and the tie-break decides Wound application, which §8.6 already excludes from v1. Increment 3's `shield-initiative` modifier and `replace-initiative-with-shield` command are deleted. `Aim` is **kept** — it is real, just sourced from Chapter 9's bow equipment rather than Chapter 7. Increment 0a surfaced both; expect Increment 0b's audit to surface more, since it is the first pass to check every named procedure against the vault. |
| D6 | What deck does Signs and Portents draw from? | **Resolved 2026-07-15** | A **fresh deck unrelated to the campaign decks**. The rulebook is silent — Ch9 says only "Refer to this table when the City Events table yields a XXI result", and never states a draw. The table's 14 rows keyed I–XIV fit either deck, because the book writes minor values as Roman numerals elsewhere (Ch7: the minor arcana "are also rated from 1–14"), so nothing in the source decides it. Rather than infer, the project owner filled the gap: Signs draws a fresh 56-card minor arcana (Ch1:201 — the Fool is *borrowed from the major arcana* into the player deck, so it is not in a fresh minor deck; such a draw always yields I–King). Expressed as `deckScope: 'fresh'` on the contract. A fresh draw is outside card conservation, never exhausts, and cannot trigger a Fool reshuffle — but is still server-authoritative and seeded. This is a **design decision filling a silence, not a quoted rule**; the audit must mark it as such. |
