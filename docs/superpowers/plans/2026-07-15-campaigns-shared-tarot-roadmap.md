...
| D7 | Where does a rule's procedure live in the book? | **Standing rule, 2026-07-15** | **At the call site, not the table.** Three separate errors shared this cause: Signs (D6), `gm-twist` (modelled as a draw when Ch10:354 says "**Glance at** the top of the minor arcana discard pile"), and the bracket convention (declared in Ch9:275's Carouse section, governing the Hangover table). Before modelling a procedure, grep for where the table is *referenced*, not only where it is printed. The audit's `source` field points at the table; it is not evidence of how the table is used. |
