...
| D3 | Is the specification approved? | **Resolved 2026-07-15** | Approved by the project owner after the review, the plan-amendment round, and the 0a import. The spec is normative for the plans; the rulebook is normative for the spec (D5). Approval does not freeze it — §8.7 already carries one post-approval amendment. |
...
| D4 | Cloudflare Pages or Workers? | **Resolved 2026-07-27 — migrate to Workers** | **Not decided on deprecation.** Cloudflare directs new investment to Workers and has said the two products converge over time, but existing Pages projects are committed to keep working; that is a tiebreaker, not a forcing function. The decisive reason is a capability gap: the rate-limit binding is absent from the Pages Functions binding list (KV, Durable Objects, R2, D1, Vectorize, Workers AI, Service, Queues, Hyperdrive, Analytics Engine, vars, secrets), confirming the Increment 5 amendment's empirical finding. On Pages every alternative is worse for its own concrete reason: a **D1 counter** writes on every request including ~1s polls, spending the same contention budget Increment 5 Task 4 must prove 30% headroom against; **KV** is ~1 write/sec per key and eventually consistent, so it is not a limiter; a **Durable Object counter** is the correct design but Pages can only *bind* to a DO and can never define one, so it requires deploying a companion Worker anyway; a **dashboard WAF rule** does work and is genuinely edge-enforced, but lives outside version control, cannot be exercised by Task 1's unit/integration suite, cannot express the plan's fail-closed behavior or `Retry-After` semantics in application code, and needs a paid WAF tier to key on the signed-in user rather than the client address. The owner's framing was to assume large scale rather than design for the expected small pilot audience, which makes the **ceiling** the deciding factor: on Workers every limiter design stays reachable — the binding now, a DO-backed globally consistent counter later — while Pages permanently forecloses the best one. Prerequisite verified: `arrowed.games` already resolves to Cloudflare nameservers (`vita`/`decker.ns.cloudflare.com`), satisfying the Workers custom-domain requirement for Cloudflare-managed NS (Pages allowed a plain CNAME; Workers does not). No adapter change is needed — `@sveltejs/adapter-cloudflare` 7.2.9 already builds for both Workers Static Assets and Pages; only `adapter-cloudflare-workers` is deprecated. The migration is configuration and deploy-target work tracked as its own piece before Increment 5 Task 1. Carry the per-location caveat into the plan: the binding is **not** a global counter. See the rewritten Increment 5 amendments. |
...
| M | `2026-07-27-cloudflare-workers-migration.md` | Pages → Workers Static Assets deployment migration (D4) | Nothing in this roadmap | Parallel; required before Increment 5 Task 1 |
| 8 | `2026-07-15-campaigns-increment-5-enablement.md` | Capacity proof, staging D1 validation, rollback rehearsal, public enablement | Increment 4, artwork, and the Workers migration | Public release |

The plans are executed in table order except the artwork plan and the Workers migration. The artwork plan may run after Increment 0b and must merge before public enablement. The migration depends on nothing in this roadmap — it touches no application code — and may run at any time, but `npx wrangler deploy --dry-run` must exit 0 on `main` before Increment 5 Task 1 begins. Do not parallelize two plans that both alter the database schema or character write path.
