 ...
 * label footer and an enlargement dialog; face-down cards render the ornate
 * back. Checked on the /deck playground, which renders the same `TarotCard`
 * component the campaign table uses, without needing a multi-client session.
 */
...
test.describe('tarot artwork rendering', () => {
	test('face-up cards render uncropped responsive art with label and enlargement dialog', async ({ page }) => {
		...
		// The draw pile is a face-down card: ornate back art, identity-free.
		const back = page.getByRole('img', { name: 'Face-down card' }).first();
		await expect(back).toBeVisible();
		const backSrcset = await back.locator('source[type="image/avif"]').first().getAttribute('srcset');
		expect(backSrcset).toContain('/tarot/rwsa/backs/ornate-240.avif 240w');
